...
class DashboardTemplateView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['res'] = Requisition.objects.filter(employee=self.request.user)
        context['resmnth'] = Requisition.objects.filter(employee=self.request.user, requisition_creation_date__lt=this_month)
        context['res_pend'] = Requisition.objects.filter(employee=self.request.user, requisition_status=1)
        context['res_pend_mnth'] = Requisition.objects.filter(employee=self.request.user, requisition_creation_date__lt=this_month, requisition_status=1)
